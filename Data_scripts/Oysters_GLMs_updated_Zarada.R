...
setwd("/Users/katiezarada/Desktop/Oysters/Analyses/Oyster_data/Transect/")
library(MASS)
library(coefplot)
library(RCurl)
...
transect = read.csv(text = tranurl)

trans=subset(transect,TransLngth!="NA")
trans$Date = as.Date(trans$Date, format = "%Y-%m-%d")
trans$Year = format(trans$Date, "%Y")

# Aggregate data to count of oysters per transect
dat=aggregate(Cnt_Live~Date+Month+ Year+Locality+Site+Bar+Substation,dat=trans,sum)
#the above adds up the counts of the oysters in each segment of transect

dat$effort=aggregate(TransLngth~Date+Month+ Year +Locality+Site+Bar+Substation,dat=trans,max)[,8]
...
hist(dat$Cnt_Live,breaks=40,freq=FALSE,col=8)
theta=c(mu=1,k=0.2)
nb_LL=function(theta)
	{
	-sum(dnbinom(dat$Cnt_Live,mu=theta[1],size=theta[2],log=TRUE))
	}
...
nb_glm=glm.nb(Cnt_Live~Locality+Site+Month+Year +offset(log(effort))+0,data=dat)
summary(nb_glm)

#This is a model of oyster density as a function of individual site
#Working with oyster counts and effort as offset

mod1=glm.nb(Cnt_Live~Site:Locality+offset(log(effort))+0,data=dat)  
...
head(trans)
LC = subset(trans, trans$Locality == "LC" & trans$Site == "O")
LCRyr=aggregate(Cnt_Live~Date+Year+Substation+Replicate + Treatment,dat=LC,sum)
LCRyr$effort=aggregate(TransLngth~Date+Year+Substation+Replicate +Treatment,dat=LC,max)[,6]
...
boxplot(LCRyr$Cnt_Live~LCRyr$Substation, col=8)

box_plot=function(substation)
{
  temp=subset(LCRyr,Substation==substation)
  boxplot(temp$Cnt_Live~temp$Year,main=paste(substation))
  abline(v=10.5)
}
par(mfcol=c(2,2))
...
full.mod<-glm.nb(Cnt_Live~Year+Treatment+Substation+offset(log(effort))-1,data=LCRyr)
summary(full.mod)
coefplot(full.mod)
