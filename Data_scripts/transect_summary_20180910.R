...
sumstats = function(x) 
  c(Mean=mean(x,na.rm=T), 
    ...
    bstrap <- c(),
    for (i in 1:1000){
      bstrap <- c(bstrap, mean(sample(x,(length(x)),replace=T), na.rm = T))},
...
#Need to use Station in the creation of CNT not station
#otherwise data from several Stations (reef elements) are combined

#Remember Station and Station are different for the LCO sites that 
#reprent epoch 3.  Only Station has the "A" and "B" designations, so that's why we work with Station.

#Get transect length per each reef element sampled
#this is super critical, here it is getting the max by Station
...
# This means as long as Station is in
# the aggregate it is aggregating to lowest level

# tran1mx<-filter(max_tran, Month=="Apr" & Year =="2013")
# max_tran2  = aggregate(TransLngth ~ Month + Year + Station,data=tran,max,na.action=na.pass)
#  
...
cnt = aggregate(Cnt_Live~ Month + Year + Replicate + Station + Site + Locality,data=tran,sum,na.rm=T,na.action=na.pass)         

cnt = merge(cnt,max_tran) 

cnt = cnt %>% arrange(Year, Month, Station)
...
table(cnt$Month,cnt$Station,cnt$Year)           
#n samples by year, locality and site

#just take a look at a few years

cnt_2014<-filter(cnt, Year == 2014)
table(cnt_2014$Month,cnt_2014$Station) 

cnt_2015<-filter(cnt, Year == 2015)
table(cnt_2015$Month,cnt_2015$Station) 

#no samples in 2016

cnt_2017<-filter(cnt, Year == 2017)
table(cnt_2017$Month,cnt_2017$Station) 
...
# Below aggregate is important. If Station is included then CI not estimated for many Stations because a lot of the Stations have only had one sampling event

stats = aggregate(Density~ Trip + Trip2 + Year + Locality + Site + Station,data=cnt,FUN=sumstats)
...
group_by(Year, Locality,Station,Season) 
...
table(LC_counts$Season,LC_counts$Station)
...
LC_winter<-filter(cnt, Season=="Winter" & 
                    Locality =="LC" & Site =="O") 
# %>% 
#   group_by(Year, Locality,Station) %>%
#   count()
...
tran.treat = tran %>% dplyr::select(Month, Year, Locality, Site, Bar, Station, Cnt_Live, Treatment, TransLngth) %>% 
  filter(Locality == "LC", Site == "O")
max_tran  = aggregate(TransLngth ~ Month + Year+ Locality+ Site+ Bar+ Station+ Treatment,data=tran.treat,max,na.action=na.pass)
cnt_treat = aggregate(Cnt_Live~ Month + Year + Locality + Site + Bar + Station +Treatment ,data=tran.treat,sum,na.rm=T,na.action=na.pass)         
